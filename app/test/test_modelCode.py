...
file = open("./test/exampleComplexModel.py", 'r')
example_staff_ebo_model_str = file.read()
file.close()

class test_modelCode(testHelperAPIClient):

  def assertAPIsInOKState(self):
    expRes = {
      "pagination": {
  ...
  @patch('githubAPICalls.githubAPICallsClass.getEBOList', return_value=[ 'AnimalsV1' ])
  @patch('githubAPICalls.githubAPICallsClass.getEBOInfoFileFromGit', return_value='{"Deployments": {"DEV_INT_TEST": "0.0.1"}}')
  @patch('githubAPICalls.githubAPICallsClass.getPythonModelFileFromGit', return_value=example_staff_ebo_model_str)
  def test_exampleStaffModel(self, getEBOListCall, getEBOInfoFileFromGit1, getEBOInfoFileFromGit2):
    self.setUpMAN()
    self.assertAPIsInOKState()
